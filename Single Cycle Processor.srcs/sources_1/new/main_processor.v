...
IF_ID if_id (clk, reset, stall, pc_out, instruction, pc_out_IF_ID, ins_out_IF_ID);
...
ID_EX id_ex (
    ...
    .pc_in(pc_out_IF_ID), .read_data1_in(rs1_data), .read_data2_in(rs2_data), .imm_input(imm),
    .rd_in(rd), .rs1_in(rs1), .rs2_in(rs2), .funct_in({ins_out_IF_ID[30], ins_out_IF_ID[14:12]}),
    .branch_in(branch), .memread_in(mem_read), .memtoreg_in(mem_to_reg),
    .memwrite_in(mem_write), .aluSrc_in(alu_src), .regwrite_in(reg_write), .Aluop_in(alu_op),
    .pc_out(pc_out_ID_EX), .rs1(rs1_ID_EX), .rs2(rs2_ID_EX), .rd(rd_ID_EX), .imm_data(imm_ID_EX),
    .readdata1(read_data1_ID_EX), .readdata2(read_data2_ID_EX), .funct_out(funct_ID_EX),
    .Branch(branch_ID_EX), .Memread(memread_ID_EX), .Memtoreg(memtoreg_ID_EX),
    .Memwrite(memwrite_ID_EX), .Regwrite(regwrite_ID_EX), .Alusrc(alusrc_ID_EX), .aluop(aluop_ID_EX)
);
hazardDetectionUnit hdu (
    .rs1_ID(rs1), .rs2_ID(rs2), .rd_EX(rd_ID_EX), .mem_read_EX(memread_ID_EX),
    .branch_ID(branch), .rd_MEM(rd_out_EX_MEM), .reg_write_EX(regwrite_ID_EX),
    .reg_write_MEM(reg_write_out_EX_MEM), .stall(stall), .flush(flush)
);
